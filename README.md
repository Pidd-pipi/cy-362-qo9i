...
- 前台收银结算：总览页内置结算操作区，选择会员后录入消费金额与抵扣积分，系统先按等级打折（青铜无折扣，白银/黄金/钻石分别 95/9/85 折），再按每 100 积分抵 10 元抵扣（不超过应付金额、积分不扣成负数），实收计入累计消费并按 1 元 1 分奖励，结算后立即展示等级变化与积分余额，并保留最近结算记录。
