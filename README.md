...
- 会员积分与等级体系：注册会员消费积累积分，设置等级规则（如青铜/白银/黄金/钻石），不同等级享受折扣与优先拼车位权益，积分可兑换周边或抵扣费用。总览页内置前台收银结算区：按会员等级自动打折（青铜不打折、白银95折、黄金9折、钻石85折），支持每100积分抵10元且抵扣不超过应付金额，实收计入累计消费（满1000/3000/8000自动升级）并按1元1分奖励积分，结算后展示原等级/新等级、折扣、实收、本次与剩余积分，并保留最近结算记录。
